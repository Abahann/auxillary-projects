...
#For each user SSH configuration, we will create an authorized_keys file and ad the below public key

#!/bin/bash
userfile=$(cat names.csv)
PASSWORD=PASSWORD

# To ensure the user running the script has sudo privilege
      if [ $(id -u) -eq 0 ]; then
...
#We need to set permission for the ssh dir
...
#We need to set permission for the key file
     su - -c "chmod ~/.ssh/authorized_keys" $user
